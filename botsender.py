...
def send_warning(type_of_message, measure, *args, **kvargs):
    ...
    """
    chat_id_token = tokens['chat_id'] #Id пользователя
    proxy_server = telegram.utils.request.Request(proxy_url=tokens['proxy_url'])
    bot = telegram.Bot(token=tokens['token'], request=proxy_server) #создаём бота
    ...
    if type_of_message == 'temperature_high':
        ...
        bot.send_message(chat_id=chat_id_token, text=text_of_message)
